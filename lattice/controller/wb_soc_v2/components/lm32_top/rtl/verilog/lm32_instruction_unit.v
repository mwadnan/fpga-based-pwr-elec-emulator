...
`ifdef CFG_IROM_ENABLED  
	...
	bram16K ram (
	  ...
	  .rsta(rst_i), // input rsta
	  .ena(!stall_a), // input ena
	  .wea(`FALSE), // input [0 : 0] wea
	  .addra(pc_a[clogb2_v1(`CFG_IROM_LIMIT/4-`CFG_IROM_BASE_ADDRESS/4+1)+2-1:2]), // input [11 : 0] addra
	  .dina({32{1'b0}}), // input [31 : 0] dina
	  .douta(irom_data_f), // output [31 : 0] douta
	  .clkb(clk_i), // input clkb
	  .rstb(rst_i), // input rstb
	  .enb(!stall_x || !stall_m), // input enb
	  .web(irom_we_xm), // input [0 : 0] web
	  .addrb(irom_address_xm[clogb2_v1(`CFG_IROM_LIMIT/4-`CFG_IROM_BASE_ADDRESS/4+1)+2-1:2]), // input [11 : 0] addrb
	  .dinb(irom_store_data_m), // input [31 : 0] dinb
	  .doutb(irom_data_m) // output [31 : 0] doutb
	);
...
// Modified MWA 05/08/13
/* Modifications for Xilinx Compatability
	1. Only make changes within the `ifdef directive for CFG_IROM_ENABLED
	2. Remove Instantiations of Lattice RAM primitive
	3. Replace with Xilinx RAM primitive (True Dual Port Block RAM) - size set according to design
*/